package com.xunheng.system.domain.login.ability.impl;
...
public class LoginDomainServiceImpl implements LoginDomainService {
    ...
    public String saLogin(LoginEntity entity) {
        ...
        /*刷新userDetail缓存*/
        userDomainService.reloadUserDetailCache(entity.getId());
        /*清除错误登录数据*/
    ...
    public void checkValidation(LoginEntity login) {
        ...
        UserEntity userLogin = userGateway.getOneByUsername(login.getUsername());
        /*账号是否存在验证*/
        if(userLogin == null)handleLoginFail(login);
        /*密码验证*/
        if(!login.checkPassword(userLogin.getPassword().getEncryptionPassWord().getPassword()))handleLoginFail(login);
        /*所属租户是否过期验证*/
        /*feign获取租户是否过期*/
        Boolean tenantExpire = tenantGateway.isTenantExpire(userLogin.getTenantId());
